...
dataset = MicrostructureDataset('MR_jpg_list.txt', transform=transform)
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.main = nn.Sequential(
            nn.Conv2d(1, 64, 4, 2, 1, bias=False), # 128x128 -> 64x64
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(64, 64 * 2, 4, 2, 1, bias=False), # 64x64 -> 32x32
            nn.BatchNorm2d(64 * 2),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(64 * 2, 64 * 4, 4, 2, 1, bias=False), # 32x32 -> 16x16
            nn.BatchNorm2d(64 * 4),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(64 * 4, 64 * 8, 4, 2, 1, bias=False), # 16x16 -> 8x8
            nn.BatchNorm2d(64 * 8),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(64 * 8, 64 * 16, 4, 2, 1, bias=False), # 8x8 -> 4x4
            nn.BatchNorm2d(64 * 16),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(64 * 16, 1, 4, 1, 0, bias=False), # 4x4 -> 1x1
            nn.Sigmoid()
        )
...
for epoch in range(num_epochs):
    ...
    if epoch == 500 or (epoch + 1) % 3000 == 0:        
        torch.save({
            'epoch': epoch,
        ...
        }, f'checkpoint_128/checkpoint_epoch_{epoch + 1}.pth')
