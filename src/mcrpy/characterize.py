...
from src.Settings import CharacterizationSettings
from src.Symmetry import Symmetry, symmetries
from src.descriptor_factory import descriptor_choices, descriptor_classes
...
def run_characterization(
    ...
    print("Running characterization in its file")
...
def characterize(
    ...
    of all possible descriptors see descriptors and the respective documentations."""
    if settings is None:
        settings = CharacterizationSettings()
    ...
    if (
        ...
        raise ValueError("""The microstructures to characterize within one call to the function
                characterize must have the same resolution, number of phases and should
                ...
                characterization intp multiple calls to characterize""")
    ...
    # load modules
    loader.load_plugins([f'descriptors.{descriptor_type}' 
        for descriptor_type in settings.descriptor_types])
    ...
    print("Made it past characterizing")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--limit_to', type=int, help='Limit in pixels to which to limit the characterization metrics.', default=16)
