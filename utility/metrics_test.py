...
def plot_pdf_cdf_bar(data, metric_name, metrics_folder, train_folder):
    print("Plotting", metric_name)
        ...
        if not fid_only:
            ...
            for percentage, train_images in conditional_train_images.items():
                for i, im in enumerate(train_images):
                    ...
                    np_array = np.array(im.convert('L'))
                    metrics["two_point_correlation_" + percentage]['train'].append(two_point_correlation.two_point_correlation(np_array))
                    metrics["pore_size_distribution_" + percentage]['train'].append(ps.metrics.pore_size_distribution(np_array))
                    metrics["lineal_path_distribution_" + percentage]['train'].append(ps.metrics.lineal_path_distribution(np_array))
                    metrics["chord_length_distribution_" + percentage]['train'].append(ps.metrics.chord_length_distribution(np_array))
            ...
            for percentage, train_images_group, eval_name in zip(percentages, train_images_groups, eval_names):
                ...
                for epoch in range(4, 27):
                    ...
                    metrics_folder = os.path.join(metrics_folder, percentage, f"eval_{epoch_multiplied}")
                    if len(eval_images) > 0:
                        act_gen = get_activations(eval_images, model, device)
                        fid = calculate_fid(act_real, act_gen)
                        fid_values.append(fid)
                        os.makedirs(metrics_folder, exist_ok=True)
                        with open(os.path.join(metrics_folder, 'fid.txt'), 'w') as f:
                            f.write(f"FID: {fid}\n")

                        print(f"FID for {percentage} images vs {eval_name} epoch_{epoch_multiplied}: {fid}")

                    print("Beginning metric calculations for test images")
                    for i, im in enumerate(eval_images):
                        print(f"test: {i+1}/{len(eval_images)}")
                        np_array = np.array(im.convert('L'))
                        metrics["two_point_correlation_" + percentage]['test'].append(two_point_correlation.two_point_correlation(np_array))
                        metrics["pore_size_distribution_" + percentage]['test'].append(ps.metrics.pore_size_distribution(np_array))
                        metrics["lineal_path_distribution_" + percentage]['test'].append(ps.metrics.lineal_path_distribution(np_array))
                        metrics["chord_length_distribution_" + percentage]['test'].append(ps.metrics.chord_length_distribution(np_array))

                    plot_metric(metrics["two_point_correlation_" + percentage]['train'], metrics["two_point_correlation_" + percentage]['test'], "Two Point Correlation", metrics_folder, conditional_train_folder)
                    plot_pdf_cdf_bar(metrics["pore_size_distribution_" + percentage], "Pore Size Distribution", metrics_folder, conditional_train_folder)
                    plot_pdf_cdf_bar(metrics["lineal_path_distribution_" + percentage], "Lineal Path Distribution", metrics_folder, conditional_train_folder)
                    plot_pdf_cdf_bar(metrics["chord_length_distribution_" + percentage], "Chord Length Distribution", metrics_folder, conditional_train_folder)
